...
describe("listUsersAction", () => {
  ...
  it("lists users", async () => {
    ...
    vi.mocked(userService.listUsers).mockResolvedValue([{ id: "user-1" } as never]);
...
describe("setUserRoleAction", () => {
  it("prevents changing own role", async () => {
    ...
    vi.mocked(userService.listUsers).mockResolvedValue([{ id: "user-1", email: "admin@t.com" } as never]);
  ...
  it("sets role for other user", async () => {
    vi.mocked(requireRole).mockResolvedValue({ id: "admin-1", email: "a@t.com", name: "A", role: "admin" });
    vi.mocked(userService.listUsers).mockResolvedValue([{ id: "user-2", email: "other@t.com" } as never]);
    vi.mocked(userService.setUserRole).mockResolvedValue({ id: "user-2" } as never);
...
describe("banUserAction", () => {
  ...
  it("bans another user", async () => {
    vi.mocked(requireRole).mockResolvedValue({ id: "admin-1", email: "a@t.com", name: "A", role: "admin" });
    vi.mocked(userService.banUser).mockResolvedValue({ id: "user-2" } as never);
...
describe("unbanUserAction", () => {
  it("unbans a user", async () => {
    vi.mocked(requireRole).mockResolvedValue({ id: "admin-1", email: "a@t.com", name: "A", role: "admin" });
    vi.mocked(userService.unbanUser).mockResolvedValue({ id: "user-2" } as never);
